...
char *db_get_xmltv_programs() {
    ...
    size_t size = 0;
    char *xml = malloc(cap);
    if (!xml) { sqlite3_finalize(stmt); return NULL; }
    xml[0] = '\0';
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        APPEND_OR_FAIL(append_str(&xml, &size, &cap, "</desc>\n"));
        /* Add tags to force Jellyfin to allow series recording */
        // Use event_id as pseudo-episode number in Season 1 to ensure unique ID
        char ep_str[64];
